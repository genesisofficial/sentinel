...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, machinecoind):
        golist = machinecoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_machinecoind(machinecoind, item)
    ...
    @classmethod
    def import_gobject_from_machinecoind(self, machinecoind, rec):
        import decimal
        ...
        # shim/machinecoind conversion
        object_hex = machinecoinlib.SHIM_deserialise_from_machinecoind(object_hex)
        objects = machinecoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from machinecoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from machinecoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from machinecoind - Machinecoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(machinecoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from machinecoind! %s" % e)
            govobj.vote_delete(machinecoind)
            return (govobj, None)
    ...
    def vote_delete(self, machinecoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(machinecoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, machinecoind, signal, outcome):
        ...
        output = machinecoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = machinecoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(machinecoind, signal)

    def sync_network_vote(self, machinecoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = machinecoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = MACHINECOIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 machinecoin addr, non-multisig
            if not machinecoinlib.is_valid_machinecoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Machinecoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (machinecoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MachinecoinDrive, etc.)
        return False
    ...
    def prepare(self, machinecoind):
        try:
            object_hash = machinecoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = MACHINECOIND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not machinecoinlib.is_valid_machinecoin_address(addr, config.network):
    ...
    def hash(self):
        import machinecoinlib
        return machinecoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = MACHINECOIND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, machinecoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - machinecoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, machinecoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - machinecoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, machinecoind):
        now = int(time.time())
        return (self.created_at < (now - machinecoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, machinecoind):
        if self.is_expired(machinecoind):
            return False

        return True

    def is_deletable(self, machinecoind):
        if self.is_expired(machinecoind):
            return True
