...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, genesisd):
        golist = genesisd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_genesisd(genesisd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_genesisd(self, genesisd, rec):
        import decimal
        import genesislib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from genesisd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from genesisd, with every run
        ...
        # get/create, then sync payment amounts, etc. from genesisd - Genesisd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(genesisd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from genesisd! %s" % e)
            govobj.vote_delete(genesisd)
            return (govobj, None)
    ...
    def vote_delete(self, genesisd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(genesisd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, genesisd, signal, outcome):
        ...
        output = genesisd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = genesislib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(genesisd, signal)

    def sync_network_vote(self, genesisd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = genesisd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = GENESISD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 genesis addr, non-multisig
            if not genesislib.is_valid_genesis_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Genesis address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Genesis Core restricts proposals to 512 bytes max
